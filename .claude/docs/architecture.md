...
- **`agents.system_prompt`/`personality` are still NULL for Malu** (C2 hasn't populated them) — `buildSystemPrompt` (step 7) falls back to composing `role + description + personality + knowledge` rather than hard-failing.
