...
export async function finishConnection(
  ...
  return { displayPhoneNumber: await lookupDisplayPhoneNumber(accessToken, phoneNumberId) };
}

export async function lookupDisplayPhoneNumber(accessToken: string, phoneNumberId: string) {
