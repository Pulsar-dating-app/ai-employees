-- Trello (K-epic summary work): content-only migration -- no schema/security
-- changes. Full rewrite of agents.system_prompt for 'ana', same shape as
-- 20260903120100_ana_waitlist_prompt. New in this pass: when booking, Ana
-- composes a short professional-facing recap of what the appointment is
-- about and passes it to book_appointment as `summary` -- it lands on the
-- dashboard card and the Google Calendar event description, so the
-- practitioner walks in knowing the context.
