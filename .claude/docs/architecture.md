...
- `src/lib/agents/catalog.ts` (new) — presentation metadata (trait chips, mocked monthly price, "what she does"/"what she'll never do" copy keys) keyed by agent slug, plus `LOCKED_AGENTS` (Emma/Support, Mia/Customer Service — named in the product spec's long-term architecture, not invented) for the marketplace's non-hireable "coming soon" cards. This is deliberately **not** DB-backed: `agents` has no columns for trait lists or tutorial copy, and locked agents have no `agents` row at all (nothing to hire yet).
...
- **Marketplace** (`src/app/dashboard/page.tsx`, was the checklist home) — a card grid: real hireable agents from `agents` (today: Malu) via `HireableAgentCard`, plus `LOCKED_AGENTS` via `LockedAgentCard`. `agent-card.tsx` holds both; the hireable card links to the agent's detail page and shows a "Hired" badge in place of the price once `company_agents` has a row for it.
