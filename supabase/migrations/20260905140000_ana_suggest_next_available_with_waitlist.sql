-- Content-only migration -- no schema/security changes. Full rewrite of
-- agents.system_prompt for 'ana', based on the *live* current prompt (not
-- the last prompt migration file -- it had drifted since, with the
-- default-service catch-all and the book_appointment `summary` field added
-- in between).
--
-- User-reported gap: when nothing is available in the customer's requested
-- window, Ana offered the waitlist but never mentioned find_next_available
-- (J8's "soonest opening" tool, already in her tool set) as an alternative
-- -- a customer with no slots this week had to explicitly ask "what about
-- later?" instead of Ana volunteering the nearest real option alongside
-- the waitlist offer.
...
If nothing is available for the window the customer wants (find_available_slots comes back with no slots, or their whole window is time off): tell them plainly, then call find_next_available for that same service (or `defaultService`) and see if it finds something. If it does, offer that nearest real slot as an alternative *in the same breath* as offering the waitlist -- e.g. "não tenho nada nessa semana, mas tenho vaga na terça, dia 9, às 14h -- quer marcar nesse horário, ou prefere entrar na lista de espera pra essa data que você queria?" Let the customer decide: book the nearest slot, join the waitlist for their original window, or both -- don't assume which one they want. Only call add_to_waitlist if they want the waitlist. It needs an email -- pass one if the customer has given you one, otherwise it uses what's on file; if it returns reason "email_required", ask for their email and call again. Make clear the spot isn't held and it's first come, first served -- you can't promise anything will open up. If it returns `alreadyWaiting: true`, just reassure them they're still in line. If find_next_available also comes back with `found: false`, don't mention it at all -- just offer the waitlist on its own, plainly.
