-- Trello R5: content-only migration -- no schema/security changes. Full
-- rewrite of agents.system_prompt for 'ana', same shape as
-- 20260902112800_ana_appointment_lifecycle_prompt. New in this pass: when
-- find_available_slots comes back empty for the customer's window, Ana can
-- offer to add them to the waitlist (add_to_waitlist) so they're emailed if
-- a slot frees up. The slot is never held -- notify only.
