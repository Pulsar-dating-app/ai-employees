...
- **Channels**: `conversation_channel` is `whatsapp` | `web_chat` | `instagram`. **Web chat** (epic M) shipped and is live. **Instagram** is the messaging channel now being built (epic N, decided 2026-08-31); its schema landed with N1. **WhatsApp** (epic D) is live again; since 2026-09-25 new numbers connect through Twilio's Partner Solution rather than Meta Cloud API direct (see architecture.md).
...
- **ProductRepository** — abstraction Malu uses to search/get products (`search(query)`, `get(productId)`) regardless of source (CSV/XLSX import and **Shopify catalogue sync** today — see `architecture.md#shopify-catalogue-connection`; WooCommerce/Nuvemshop/VTEX/API later). Malu must never know where products came from — a Shopify-synced row is just a `products` row with `external_id` `"shopify:<id>"` and `metadata.source = "shopify"`.
