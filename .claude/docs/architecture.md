...
`src/app/page.tsx` no longer redirects an unauthenticated visitor straight to `/login` — it renders the real marketing page (`src/components/landing/`) instead; a signed-in visitor still redirects to `/dashboard`. First Persuade-mode surface in the app (everything else is Operate); deliberately a **separate, scoped visual world** from the dashboard rather than reusing its exact tokens.

- **Design system**: `.landing`-scoped CSS custom properties in `globals.css` (`--landing-bg`/`-ink`/`-sub`/`-line`/`-coral`/`-coral-ink`/`-coral-tint`/`-indigo`/`-indigo-ink`) — same coral/indigo brand family as the dashboard, kept in its own namespace so it can't leak into or fight `--color-accent-*`. Typography is Manrope (display) + Karla (body), loaded only by `src/components/landing/fonts.ts` (`next/font/google`) so the dashboard's Geist bundle is untouched. The palette went through two rounds of direct feedback — bolder/more illustrative first, then converged toward restraint after the user pointed at a competitor (widde.io) as a "more professional" reference — see decisions.md for both.
- **`src/components/landing/whatsapp-demo.tsx`** — the page's central proof device: a real phone-frame mockup that types out an actual conversation from `Sidde_MVP_Specification.md` §9 character-by-character, not a static screenshot. **Real bug fixed here**: the first version guarded its play-once animation with a `useRef(false)` "already played" flag, which defeats React Strict Mode's dev-only double-invoke of effects (mount → cleanup → mount again) — the first pass got cancelled by the synthetic cleanup almost immediately, and the guard then blocked the second, real pass from ever starting, so the demo looked completely static. Fix: drop the ref guard entirely and rely only on a per-run `cancelled` flag closed over inside the effect, which makes the effect safely re-runnable — the correct pattern for Strict Mode, not a special case. **How to apply**: any future mount-driven animation effect (not just here) must be idempotent/cleanup-safe on its own; never add an `"already ran"` ref to silence Strict Mode's double-invoke, since that's exactly the double-invoke's job to catch.
- **`src/components/landing/agent-gallery.tsx`** — shows the live, real agent roster (today: Malu and John), not a hardcoded or fictional lineup — the same "pull from the database" correction the dashboard marketplace already went through (see decisions.md's 2026-08-26 entry on that). **Uses `createServiceClient()`, not the regular RLS-scoped client**: `agents`' RLS policy grants `SELECT` to `authenticated` only, and this page renders for anonymous visitors, so the regular client would silently return zero rows here — same reasoning as `ProductRepository` (B5): a caller with no session reading data that's meant to be public needs the service client, with `is_active`/company-scoping (n/a here) enforced in application code instead of by RLS. A trailing "more on the way" card states the long-term multi-agent vision honestly as vision, never as a current-capability claim.
- **`src/components/landing/typewriter-text.tsx`** / **`rotating-word.tsx`** — small, reusable, purposeful motion primitives (not decoration): the hero headline types itself out once on mount, and a single word within the hero's second line cycles through a short list of real capabilities (understands/recommends/sells/responds) via a vertical morph. Both respect `prefers-reduced-motion` and keep an `aria-hidden` animated copy alongside a plain `sr-only` full-text fallback so a screen reader never gets the partial/rotating state.
- **No fabricated commercial content**: no invented price is shown publicly (the dashboard's mocked hire-flow price stays internal, disclosed there as a preview), no testimonials, no client logos, no usage stats — PRODUCT.md's Evidence on Hand section forbids all of these, and the page's credibility instead leans on the real conversation demo, the real agent roster, and honestly-labeled vision language.
