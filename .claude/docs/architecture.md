...
- `src/app/dashboard/dev-whatsapp-connect-test/` — a **temporary, dev-only**
  ...
  system, guarded to `notFound()` in production) that loads the Facebook JS
  ...
  **The production guard must live inside the component body, not at module
  scope** — an earlier version `throw`'d at the top of the file when
  `NODE_ENV === "production"`, which broke `next build` outright (page-data
  collection runs with `NODE_ENV=production` regardless of whether the
  route is ever linked to), even though nothing in production ever
  navigates here. Fixed to call `notFound()` inside the component function
  instead (Trello F3's build-verification pass). The same bug existed in
  `manual-connect-test` below (a redundant module-level `throw` on top of
  its own already-correct in-handler 404 check) and was removed there too —
  any future dev-only route's production gate belongs inside the
  handler/component, never at module scope.
...
- **Dashboard entry point**: a new `LinkStepCard` ("Manage your product catalog") in `src/app/dashboard/page.tsx`, with its own `status` (`done` once any product exists for the company, active or not, else `active`) — but it does **not** increment `totalSteps`/`completedSteps`. Spec §4's onboarding flow is fixed at 4 named steps (Hire → Teach → Connect → Ready); catalog management isn't one of them.
